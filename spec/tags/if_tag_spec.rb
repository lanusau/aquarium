...
describe Aquarium::Tags::If do
  ...
  describe '#new' do
      it 'creates tag object' do
        ...
        expect(Aquarium::Tags::If.new('select count(*) from dba_tablespaces','file_name',@change_collection)).to be_an_instance_of(Aquarium::Tags::If)
  ...
  describe '#parse' do
    context 'when #if tag is terminated by #endif tag' do
      it 'extracts SQLs for conditional SQL collection' do
        ...
        @change_collection.add_change(@change)
        Aquarium::Tags::If.new('select count(*) from dba_tablespaces','file_name',@change_collection).parse(file)
    ...
    context 'when #if tag is not terminated properly' do
      it 'raises an error' do
        file = StringIO.new("select * from table1\n;\nselect * from table2\n;\n--#blah")
        @change_collection.add_change(@change)
        expect {Aquarium::Tags::If.new('select count(*) from dba_tablespaces','file_name',@change_collection).parse(file)}.to raise_error
      end
    end
  end
end